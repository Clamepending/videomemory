...
STACK_COMPOSE_FILE="${STACK_COMPOSE_FILE:-docker-compose.simpleagent.yml}"
if [[ ! -f "$STACK_COMPOSE_FILE" ]]; then
  STACK_COMPOSE_FILE="docker-compose.adminagent.yml"
fi

docker compose -f "$STACK_COMPOSE_FILE" up -d --build
