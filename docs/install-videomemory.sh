...
log "Docker OpenClaw prompt: Please inspect 'npx -y @clamepending/videomemory@0.1.2 onboard --safe --repo-ref v0.1.2 --videomemory-base $DOCKER_OPENCLAW_BASE --explain'. If it looks safe, run 'npx -y @clamepending/videomemory@0.1.2 onboard --safe --repo-ref v0.1.2 --videomemory-base $DOCKER_OPENCLAW_BASE'. Then use VideoMemory from $DOCKER_OPENCLAW_BASE/openclaw/skill.md and use the videomemory task helper for any 'when X happens, do Y' request."
